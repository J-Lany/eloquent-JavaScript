...
Помните, что у каждого объекта, описывающего систему письма, есть свойство direction, 
принимающее значения "ltr" (left to right, «слева направо»), "rtl" (right to left, «справа налево») 
или "ttb" (top to bottom, «сверху вниз»).
Доминирующее направление — это направление большинства символов, которые принадлежат конкретной системе письма. 
Вероятно, вам пригодятся описанные в этой главе функции characterScript и countBy.
*/

function checkDirection(string) {
  const counts = countBy(string, characterScript);
...
//Проверка:
console.log(checkDirection('Daniella джейлани'));
